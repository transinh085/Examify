...
const RecentQuizItem = ({ id, title, description, cover, questionCount, currentProgress }) => {
  const twoColors = {
    '0%': '#108ee9',
    '100%': '#87d068',
  };
    ...
    <Link to={`/join/game/${id}`}>
      ...
      >
        ...
        <Flex vertical justify="space-between" className="p-3 h-[130px]" gap={2}>
          ...
          <Progress percent={currentProgress} size={[null, 12]} status="active" strokeColor={twoColors} />
        </Flex>
      </Card>
    </Link>
  );
